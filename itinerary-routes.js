...
async function callDashScopeItinerary(systemPrompt, userBlock) {
...
async function fetchContextEvents(supabase, city) {
  const c = String(city || '').trim();
  if (!c) return [];
  ...
  const [cRes, vRes] = await Promise.all([
    supabase.from('events_chatbot').select(sel).ilike('city', pattern).limit(45),
    supabase.from('events_chatbot').select(sel).ilike('venue', pattern).limit(45),
  ]);
...
function buildUserPrompt({
  ...
  return [
    ...
    `Trip length: ${tripDays} calendar day(s).`,
    `GEO STRATEGY:\n${geoGuidance}`,
    ...
    `Other events in/near **${city}** (context only): ${JSON.stringify(contextEvents.slice(0, 32))}`,
...
/** Three parallel AI runs — accents must stay distinct enough for genuine choice. */
const ITIN_VARIANT_PRESETS = [
  {
    key: 'signature',
    title: 'Signature circuit',
    tagline: 'Iconic landmarks & definitive sights',
    accent:
      'Prioritise well-known viewpoints, emblematic neighbourhoods, and the “classic Malaysia” itinerary a first-timer would expect. Named landmarks beat obscure holes-in-the-wall.',
  },
  {
    key: 'culture',
    title: 'Culture & flavour',
    tagline: 'Heritage, craft & standout food',
    accent:
      'Prioritise museums, heritage quarters, markets, hawker gems, artisan cafés and cultural layers. Prefer depth and food discovery over marquee photo stops.',
  },
  {
    key: 'ease',
    title: 'Quiet rhythm',
    tagline: 'Slower pacing & understated favourites',
    accent:
      'Prioritise calm pacing regardless of nominal “pace” setting—leafy strolls, tasteful cafes, spa or nature pockets, locals’ understated favourites; avoid cramming slots.',
  },
];
...
async function finalizeItineraryFromParsed(parsed, ctx) {
  ...
  let placesFull = await hydratePlaceImages(placesArr, city, unsplashClientIdFromEnv());
  const googlePlacesKey = process.env.GOOGLE_PLACES_API_KEY || '';
  const placesLive = await enrichPlacesWithGooglePlaces(placesFull, city, stateName, googlePlacesKey);
  ...
  appendPlacesLiveAlerts(warnings, placesFull, Boolean(googlePlacesKey));
  ...
  return {
    ...
    places: placesFull,
    travelLinks,
    _placesLiveReasons: placesLive.reasons,
    _matched: placesLive.matched,
...
async function regenerateSingleDay(sb, opts) {
  ...
  let raw;
  raw = await callDashScopeItinerary(DAY_REGEN_SYSTEM, userBlock);
...
function registerItineraryRoutes(app, deps) {
  ...
  app.get('/itinerary-modal.js', (req, res) => {
    res.type('application/javascript');
    res.sendFile(require('path').join(__dirname, 'itinerary-modal.js'));
  });
  ...
  app.post('/api/itinerary/generate', async (req, res) => {
    ...
    const tripDays = dates.length;
    const geoGuidance = geoGuidanceForTrip(tripDays, city, stateName);
    const contextEvents = await fetchContextEvents(sb, city);
    let contextEventsNational = [];
    if (tripDays >= 6) {
      contextEventsNational = await fetchEventsDuringTripWindow(sb, arrivalDate, departureDate, 35);
    }
    ...
    const settled = await Promise.allSettled(
      ITIN_VARIANT_PRESETS.map((preset) =>
        callDashScopeItinerary(
          ITIN_SYSTEM,
          buildUserPrompt({
            ...promptCtx,
            variantAccent: preset.accent,
          }),
        )
          .then((txt) => {
            const p = parseLlmJson(txt);
            if (!p || typeof p !== 'object' || !Array.isArray(p.places) || p.places.length === 0) {
              throw new Error('Bad itinerary JSON');
            }
            return finalizeItineraryFromParsed(p, {
              dates,
              city,
              stateName,
              tripDays,
    ...
    for (let i = 0; i < settled.length; i++) {
      const r = settled[i];
      const preset = ITIN_VARIANT_PRESETS[i];
      if (r.status === 'fulfilled') {
        variants.push(r.value);
      } else {
        console.warn(`[itinerary] variant ${preset.key} failed:`, r.reason?.message || r.reason);
      }
    }
    ...
    if (variants.length === 0) {
      return res.status(502).json({
        error:
          'Could not generate itineraries (all variants failed). Try again, shorten the trip window, or check DASHSCOPE_ITINERARY_TIMEOUT_MS.',
      });
    }
    if (variants.length < ITIN_VARIANT_PRESETS.length) {
      warnings.push({
        type: 'partial_variants',
        severity: 'info',
        message: `Returned ${variants.length} of ${ITIN_VARIANT_PRESETS.length} curator styles — retry later for a full trio if one draft failed.`,
      });
    }
